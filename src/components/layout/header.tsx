              ...
              <DrawerContent className="min-h-dvh">
                ...
                <div className="px-6 flex flex-col gap-4">
                  {links.map((link) => (
                    ...
                    >
                      {t(link.title as 'nav.home' | 'nav.experience' | 'nav.projects' | 'nav.contact')}
                      {link.isComingSoon && (
...
const HeaderLink = ({ title, href }: { title: string; href: string }) => {
  ...
  return (
    ...
    >
      <Link href={href}>{t(title as 'nav.home' | 'nav.experience' | 'nav.projects' | 'nav.contact')}</Link>
    </div>
  )
}
